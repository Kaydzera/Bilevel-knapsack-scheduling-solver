"""Convert cached test_big instances using strict hand-crafted MPS/AUX style.

This exporter targets compatibility with MibS parser behavior observed in the
known-good hand-crafted files.
"""
...
from formulation.mps_generator import generate_mps_aux_files_strict_template


ROOT = Path(__file__).resolve().parents[2]
CACHE_PATH = ROOT / "gurobi_scheduling" / "enumeration_results_cache_big.json"
OUT_DIR = Path(__file__).resolve().parent / "test_output" / "test_big_cache_mibs_strict"
...
def main(limit: int = 10) -> None:
    ...
    for idx, (cache_key, cache_entry) in enumerate(cache.items()):
        ...
        try:
            ...
            mps_path, aux_path = generate_mps_aux_files_strict_template(instance, str(OUT_DIR))
...
if __name__ == "__main__":
    cli_limit = int(sys.argv[1]) if len(sys.argv) > 1 else 10
    main(cli_limit)
